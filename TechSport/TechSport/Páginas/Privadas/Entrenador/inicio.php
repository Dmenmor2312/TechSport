<?php
// CORREGIR PROBLEMA DE CARÁCTER "s" - Agregar buffer de salida
...
// Obtener equipo y eventos CON LA NUEVA FUNCIÓN CORREGIDA
$equipo = obtenerEquipoUsuario($conn, $usuario_id, $rol_id);

// Para diagnóstico: obtener información detallada
...
$eventos_proximos = obtenerEventosProximos($eventos);

// Verificar permisos
$puede_gestionar_completo = $equipo ? puedeGestionarEquipoCompleto($conn, $usuario_id, $equipo['id']) : false;
...
<head>
    ...
    <title>TechSport - Inicio Entrenador</title>
...
<body data-rol="entrenador">
    ...
    <main>
        ...
        <div class="welcome-section">
            ...
            <p>Panel de control del entrenador</p>
        </div>

        <!-- Diagnóstico si no tiene equipo -->
        <?php if (!$equipo): ?>
            <div class="alert alert-warning">
                <h3>⚠️ No tienes un equipo asignado</h3>

                <?php if (!empty($info_entrenador)): ?>
                    <p><strong>Información de entrenador encontrada:</strong></p>
                    <ul style="text-align: left;">
                        <li>Equipo: <strong><?php echo htmlspecialchars($info_entrenador['equipo_nombre']); ?></strong></li>
                        <li>Tipo: <strong>Entrenador <?php echo $info_entrenador['es_principal'] ? 'Principal' : 'Auxiliar'; ?></strong></li>
                        <?php if (!$info_entrenador['es_principal'] && isset($info_entrenador['titulo_entrenador'])): ?>
                            <li>Título: <?php echo htmlspecialchars($info_entrenador['titulo_entrenador']); ?></li>
                        <?php endif; ?>
                        <?php if (!$info_entrenador['es_principal'] && isset($info_entrenador['nombre_entrenador_principal'])): ?>
                            <li>Entrenador Principal: <?php echo htmlspecialchars($info_entrenador['nombre_entrenador_principal']); ?></li>
                        <?php endif; ?>
                        <li>Estado: <?php echo isset($info_entrenador['activo']) ? ($info_entrenador['activo'] ? 'ACTIVO' : 'INACTIVO') : 'ACTIVO'; ?></li>
                    </ul>
                    <p><strong>Problema detectado:</strong> La función principal no está recuperando tu equipo correctamente.</p>
                    <p>Contacta con el administrador para revisar la configuración.</p>
                <?php else: ?>
                    <p>No se encontró información de entrenador en el sistema.</p>
                    <p>Puedes:</p>
                    <ol style="text-align: left; margin-left: 20px;">
                        <li>Crear un nuevo equipo como entrenador principal</li>
                        <li>Pedir a otro entrenador que te agregue como auxiliar a su equipo</li>
                    </ol>
                <?php endif; ?>

                <div style="margin-top: 20px;">
                    <a href="/TechSport/Páginas/Privadas/Entrenador/GestionEquipo.php" class="btn-action btn-primary">
                        🏆 Crear Nuevo Equipo
                    </a>
                </div>
            </div>
        <?php else: ?>
            ...
            <div class="team-info-card">
                ...
                <?php if (isset($equipo['tipo_entrenador'])): ?>
                    <p>👤 Rol: <strong>Entrenador <?php echo $equipo['tipo_entrenador'] == 'principal' ? 'Principal' : 'Auxiliar'; ?></strong></p>
                <?php endif; ?>
                <?php if (!$puede_gestionar_completo && isset($equipo['tipo_entrenador']) && $equipo['tipo_entrenador'] == 'auxiliar'): ?>
                    <p class="info-note">ℹ️ Eres entrenador auxiliar. Algunas funciones están limitadas.</p>
                <?php endif; ?>
            </div>
            ...
            <div class="button-section">
                <?php if ($puede_gestionar_completo): ?>
                    <!-- Botones para entrenador principal -->
                    <a href="/TechSport/Páginas/Privadas/EditarCalendario.php" class="btn-action btn-primary">
                        🗓️ Editar Calendario
                    </a>
                    <a href="/TechSport/Páginas/Privadas/Entrenador/GestionEquipo.php" class="btn-action btn-secondary">
                        👥 Gestionar Equipo
                    </a>
                <?php else: ?>
                    <!-- Botones para entrenador auxiliar -->
                    <a href="/TechSport/Páginas/Privadas/EditarCalendario.php" class="btn-action btn-primary">
                        🗓️ Editar Calendario
                    </a>
                    <a href="/TechSport/Páginas/Privadas/Entrenador/GestionEquipo.php" class="btn-action btn-secondary">
